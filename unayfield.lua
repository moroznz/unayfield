--[[
	...
	- Removed all analytics/telemetry (no external HTTP calls)
	- Fixed: #file > 0 settings check (always false for dicts) -> next() check
	- Fixed: keyFileContents undefined variable in Key System
	- Fixed: useMobilePrompt/toDestroy global variable leaks -> local
	- Fixed: deprecated :connect() -> :Connect() in ColorPicker
	- Fixed: Settings topbar button used TabButton out of scope
	- Fixed: Destroy() nil-guard on hideHotkeyConnection
	- Fixed: Duplicate icon-loading block in CreateLabel
	- Fixed: game:Shutdown() after Kick (unnecessary, can error)
	- Changed: ConfigurationExtension .rfld -> .unf
...
local ConfigurationExtension = ".unf"
local settingsTable = {
	General = {
...
repeat
	...
	local toDestroy, Rayfield = Rayfield, useStudio and script.Parent:FindFirstChild('Rayfield') or game:GetObjects("rbxassetid://10804731440")[1]
...
Topbar.Search.MouseButton1Click:Connect(function()
	...
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			for _, OtherTabButton in ipairs(TabList:GetChildren()) do
				if OtherTabButton.Name ~= "Template" and OtherTabButton.ClassName == "Frame" and OtherTabButton.Name ~= "Placeholder" and OtherTabButton.Name ~= "Unayfield Settings" then
					TweenService:Create(OtherTabButton, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.TabBackground}):Play()
					TweenService:Create(OtherTabButton.Title, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {TextColor3 = SelectedTheme.TabTextColor}):Play()
					TweenService:Create(OtherTabButton.Image, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageColor3 = SelectedTheme.TabTextColor}):Play()
					TweenService:Create(OtherTabButton, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.7}):Play()
...
-- commented out: loadWithTimeout for external boost.lua (analytics-free fork — not needed)
