--[[
	...
	Unayfield Interface Suite
	by morozzz

	Based on Rayfield by Sirius
	Fork: analytics-free, no external requests

	═══════════════════════════════════════════════════════
	СПИСОК ИСПРАВЛЕНИЙ (относительно оригинальной версии):
	═══════════════════════════════════════════════════════

	1. loadSettings(): #file на dict-таблице всегда 0 — настройки никогда не загружались.
	   Исправлено: next(file) ~= nil для проверки непустоты словаря.

	2. loadSettings(): `for k, v in overriddenSettings` — генерирует синтаксическую ошибку
	   в стандартном Lua (без generalized for). Исправлено: pairs(overriddenSettings).

	3. useMobilePrompt: переменная использовалась без объявления local — была глобальной,
	   что могло конфликтовать с другими скриптами на том же executor'е.
	   Дополнительно: присваивание было ДО объявления local (строки переставлены).

	4. KeySystem: `keyFileContents` — необъявленная переменная (всегда nil), из-за чего
	   блок проверки сохранённого ключа никогда не выполнялся → сохранённые ключи не принимались.
	   Исправлено: читаем файл напрямую перед проверкой.

	5. ColorPicker FocusLost: после ввода HEX-цвета Color сохранялся как
	   Color3.fromRGB(h*255, s*255, v*255) — это HSV-компоненты, не RGB.
	   Исправлено: Color сохраняется непосредственно из rgbColor (результата парсинга HEX).

	6. ColorPicker: RunService.RenderStepped:connect() (устаревший .connect!) никогда
	   не отключался — постоянная утечка памяти и CPU-нагрузка.
	   Исправлено: соединение сохраняется, отключается в Destroying.

	7. ColorPicker RGB-инпуты: использовался deprecated API .connect вместо .Connect.

	8. CreateSlider: формула начального размера прогресса использовала (Value + Min) вместо
	   (Value - Min) — для ненулевого Range[1] полоса показывалась неверно.
	   Та же проблема в SliderSettings:Set. Исправлено в обоих местах.

	9. Кнопка Settings (Topbar): обработчик использовал переменную `TabButton` из замыкания
	   последнего вызова CreateTab — неверный таб деселектировался.
	   Исправлено: фильтрация по имени "Unayfield Settings".

	10. InputSettings:Set: pcall для callback вызывался, но результат игнорировался —
	    ошибки callback не показывались. Исправлено с полной обработкой ошибок.

	11. CreateLabel: блок установки иконки дублировался дважды подряд.
	    Исправлено: дублирование убрано, код вынесен в один блок.

	═══════════════════════════════════════════════════════
	ДОПОЛНИТЕЛЬНЫЕ ИСПРАВЛЕНИЯ (найдены при ревью):
	═══════════════════════════════════════════════════════

	12. loadSettings(): outer pcall оборачивал task.spawn — бессмысленно, task.spawn
	    никогда не пробрасывает ошибки наружу. Реальный pcall перенесён внутрь.

	13. loadSettings(): assert() внутри task.spawn тихо крашил спавн при неверном имени
	    настройки. Заменён на warn() + проверку #split ~= 2.

	14. CreateTab: TabPage.LayoutOrder = #GetChildren() or Ext and 10000 — неверный
	    приоритет операторов, Ext and 10000 никогда не применялся.
	    Исправлено: (Ext and 10000) or #GetChildren().

	15. DropdownSettings:Set: SaveConfiguration() был закомментирован — Set не сохранял
	    состояние дропдауна в конфиг, в отличие от всех остальных элементов.

	16. RayfieldLibrary:Destroy(): hideHotkeyConnection:Disconnect() без nil-проверки —
	    краш если Destroy() вызван до полной инициализации. Добавлена nil-проверка.

	17. Topbar.Hide кнопка: setVisibility(Hidden, ...) — логика инвертирована.
	    Кнопка Hide передавала текущее Hidden вместо not Hidden, т.е. пыталась
	    повторно скрыть уже скрытый UI. Исправлено: setVisibility(not Hidden, ...).

	18. CreateWindow: двойной if Settings.Icon — внутренний всегда true внутри внешнего,
	    else-ветка (rbxassetid://0) никогда не выполнялась. Убран лишний if.

	19. getIcon(): error() заменён на warn() + return nil — неверное имя иконки
	    не должно крашить весь скрипт разработчика. Все вызовы защищены nil-проверкой.

]]

if debugX then
	warn('Initialising Unayfield')
end
...
-- getService: безопасное получение сервиса Roblox.
-- cloneref используется exploit-окружениями для защиты от обнаружения через anti-cheat.
...
local settingsTable = {
	General = {
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'Unayfield Keybind'},
	},
}
...
-- prompt not loaded from external URL in Unayfield
if not prompt then
	prompt = {
		create = function() end -- No-op
	}
end


-- callSafely: обёртка pcall для функций файловой системы.
-- Файловые функции (writefile, readfile и т.д.) могут бросать ошибки если:
--   - executor не поддерживает их
--   - нет прав на запись
--   - путь недопустим
-- Возвращает false при ошибке, иначе — результат функции.
local function callSafely(func, ...)
	if func then
		local success, result = pcall(func, ...)
		if not success then
			warn("Unayfield | Function failed with error: ", result)
			return false
		else
			return result
		end
...
-- loadSettings: загружает настройки интерфейса (keybind и т.д.) из файла.
-- Вызывается ДВАЖДЫ:
--   1. При инициализации (settingsCreated = false) — кешируется в cachedSettings
--   2. После createSettings() (settingsCreated = true) — применяется к UI-элементам
-- Это сделано потому что UI-элементы настроек создаются позже самих настроек.
local function loadSettings()
	local file = nil

	-- ИСПРАВЛЕНИЕ: убрана бессмысленная обёртка outer-pcall вокруг task.spawn.
	-- task.spawn никогда не бросает исключений наружу — ошибки внутри спавна
	-- замалчиваются. Оборачиваем только реальные файловые операции.
	task.spawn(function()
		local ok, err = pcall(function()
			...
			if useStudio then
				file = [[
	{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"Unayfield Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Unayfield Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}}}
]]
			end
			...
			-- ИСПРАВЛЕНИЕ: #file на словаре (dict) всегда возвращает 0, поэтому настройки
			-- никогда не загружались из файла. Используем next() для проверки непустоты таблицы.
			if next(file) ~= nil then
				-- Файл настроек не пустой — применяем сохранённые значения
				for categoryName, settingCategory in pairs(settingsTable) do
					if file[categoryName] then
						for settingName, setting in pairs(settingCategory) do
							if file[categoryName][settingName] then
								setting.Value = file[categoryName][settingName].Value
								-- Применяем с учётом возможных overrides разработчика
								setting.Element:Set(getSetting(categoryName, settingName))
							end
						end
					end
				end
			-- Файл пустой или не существовал — применяем только overriddenSettings
			else
				for settingName, settingValue in pairs(overriddenSettings) do
					local split = string.split(settingName, ".")
					-- Имя должно иметь формат "Категория.Настройка"
					if #split ~= 2 then
						warn("Unayfield | Invalid overridden setting name: " .. settingName)
					else
		...
		if not ok then
			if writefile then
				warn('Unayfield had an issue accessing configuration saving capability: ' .. tostring(err))
			end
		end
	end)
end
...
local sendReport = function(ev_n, sc_n) end -- analytics removed in Unayfield
...
repeat
	...
	if not warned then
		warn('Unayfield | Build Mismatch')
		print('Unayfield may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end
...
if useStudio then
	...
	local ok, result = pcall(function()
		return loadstring(game:HttpGet('https://raw.githubusercontent.com/moroznz/unayfield/main/icons.lua'))()
	end)
	if ok then
		Icons = result
	else
		warn("Unayfield | Failed to load icons: " .. tostring(result))
	end
end
-- Variables
...
-- useMobilePrompt: true = показывать тап-кнопку "Show" вместо текста с клавишей.
-- ИСПРАВЛЕНИЕ: переменная не была объявлена как local — работала как глобальная,
-- что могло вызывать конфликты с другими скриптами на том же executor'е.
-- Присваивание перенесено сюда (после объявления local), убрана раздвоенная запись в двух местах.
local useMobilePrompt = UserInputService.TouchEnabled and true or false
local Notifications = Rayfield.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when Rayfield is destroyed

local SelectedTheme = RayfieldLibrary.Theme.Default

-- ChangeTheme: применяет тему ко всему интерфейсу.
-- Принимает либо строку (название из RayfieldLibrary.Theme), либо таблицу с кастомными цветами.
-- Обновляет цвета всех текстовых и фреймовых элементов через GetDescendants.
...
local function getIcon(name : string): {id: number, imageRectSize: Vector2, imageRectOffset: Vector2}
	...
	if not r then
		-- УЛУЧШЕНИЕ: warn вместо error — плохое имя иконки не должно крашить весь скрипт
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Unayfield | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Unayfield | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end

-- makeDraggable: добавляет drag-функциональность к объекту.
-- object       - перемещаемый объект (Main-фрейм)
-- dragObject   - объект за который тянуть (Topbar или Drag-бар)
-- enableTaptic - true = анимировать drag-бар при захвате
-- tapticOffset - вертикальное смещение drag-бара относительно Main
--                формат: {десктоп_offset, мобильный_offset}
...
-- PackColor / UnpackColor: конвертация Color3 в/из JSON-сериализуемую таблицу {R, G, B}.
-- Значения в диапазоне [0, 255] (не [0, 1]).
-- Используется для сохранения цветов ColorPicker в конфигурационный файл.
...
-- LoadConfiguration: читает JSON-файл конфигурации и применяет значения флагов к UI-элементам.
-- Флаги — это идентификаторы (Flag = "Toggle1"), которые разработчики задают при создании элементов.
-- Возвращает true если хотя бы одно значение изменилось.
local function LoadConfiguration(Configuration)
	local success, Data = pcall(function() return HttpService:JSONDecode(Configuration) end)
	local changed

	if not success then warn('Unayfield had an issue decoding the configuration file, please try delete the file and reopen Unayfield.') return end
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Unayfield | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--RayfieldLibrary:Notify({Title = "Unayfield Flags", Content = "Rayfield was unable to find '"..FlagName.. "' in the save file. Check github.com/morozzz for help.", Image = 3944688398})
		end
	end
...
-- SaveConfiguration: сохраняет текущие значения всех флагов в JSON-файл.
-- Вызывается при каждом изменении элемента (toggle, slider, keybind и т.д.).
-- CEnabled должен быть true (ConfigurationSaving.Enabled) иначе сохранения не происходит.
...
-- Notify: показывает всплывающее уведомление в правом нижнем углу.
-- data (таблица):
--   Title    - заголовок уведомления
--   Content  - текст уведомления
--   Image    - иконка (число = asset ID, строка = Lucide имя)
--   Duration - время отображения в секундах (по умолчанию: авто по длине текста)
	...
	task.spawn(function()
		...
		if data.Image then
			if typeof(data.Image) == 'string' and Icons then
				...
				-- УЛУЧШЕНИЕ: защита от nil если иконка не найдена
				if asset then
		...
		if data.Actions then
			warn('Unayfield | Not seeing your actions in notifications?')
			print("Notification Actions are being sunset for now, keep up to date on when they're back in the discord. (github.com/morozzz)")
		end
...
-- Hide: скрывает весь интерфейс (Main + элементы).
-- notify = true → показывает уведомление с подсказкой как вернуть интерфейс.
-- После скрытия Hidden = true. Используется как по хоткею, так и через кнопку Hide.
...
-- Maximise: разворачивает интерфейс обратно из свёрнутого состояния.
...
-- Unhide: показывает интерфейс обратно после Hide().
-- Если интерфейс был минимизирован перед скрытием — вызывает Maximise().
...
-- Minimise: сворачивает интерфейс до одной строки топбара (скрывает контент, но не весь UI).
-- Minimised = true. Кнопка ChangeSize переключает между Minimise/Maximise.
...
-- createSettings: создаёт встроенный таб "Unayfield Settings" в конце списка вкладок.
-- Доступен только если executor поддерживает файловую систему (writefile/isfile и т.д.).
-- Содержит keybind для открытия/закрытия интерфейса (и другие будущие настройки).
local function createSettings(window)
	...
	local newTab = window:CreateTab('Unayfield Settings', 0, true)

	if TabList['Unayfield Settings'] then
		TabList['Unayfield Settings'].LayoutOrder = 1000
	end

	if Elements['Unayfield Settings'] then
		Elements['Unayfield Settings'].LayoutOrder = 1000
	end
...
-- CreateWindow: главная точка входа для разработчика.
-- Settings (таблица):
--   Name            - заголовок окна
--   LoadingTitle    - текст во время загрузки
--   LoadingSubtitle - подзаголовок во время загрузки
--   Icon            - иконка в топбаре (число = asset ID, строка = Lucide имя)
--   Theme           - тема ('Default', 'Ocean', 'DarkBlue' и т.д.)
--   ToggleUIKeybind - клавиша для скрытия/показа (строка или Enum.KeyCode)
--   ConfigurationSaving = { Enabled, FileName, FolderName }
--   KeySystem       - включить систему ключей
--   KeySettings     - настройки ключевой системы
-- Возвращает объект Window с методом CreateTab.
function RayfieldLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RayfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'Unayfield may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Unayfield"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Unayfield Interface Suite" then
		LoadingFrame.Version.Text = "Unayfield"
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Unayfield | "..Key.." Error " ..tostring(Response))
					warn('Check github for help with Unayfield specific development.')
				end
			end
		end
		...
		if callSafely(isfile, RayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			-- ИСПРАВЛЕНИЕ: ранее использовалась необъявленная переменная `keyFileContents` (всегда nil),
			-- из-за чего условие никогда не выполнялось и сохранённый ключ не принимался.
			-- Теперь читаем файл один раз и проверяем каждый допустимый ключ.
			...
			if savedKeys then
				for _, MKey in ipairs(Settings.KeySettings.Key) do
					-- plain=true чтобы не интерпретировать ключ как Lua-паттерн
					if string.find(savedKeys, MKey, 1, true) then
						Passthrough = true
					end
				end
			end
	...
	local Window = {}
	-- CreateTab: создаёт новую вкладку в интерфейсе.
	-- Name  - отображаемое имя вкладки (также используется как идентификатор)
	-- Image - иконка вкладки (число = asset ID, строка = Lucide имя иконки, 0 = без иконки)
	-- Ext   - если true, вкладка скрыта из TabList (используется для системных вкладок)
	-- Возвращает объект Tab с методами CreateButton, CreateToggle, CreateSlider и т.д.
	function Window:CreateTab(Name, Image, Ext)
		...
		-- ИСПРАВЛЕНИЕ: приоритет операторов — `#GetChildren() or (Ext and 10000)` не работал,
		-- т.к. #GetChildren() никогда не falsy. Правильный порядок: сначала Ext, потом fallback.
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Unayfield | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check github for help with Unayfield specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.HexInput.InputBox.FocusLost:Connect(function()
				if not pcall(function()
						...
						setDisplay()
						-- ИСПРАВЛЕНИЕ: сохраняем как RGB-цвет, а не пересчитываем из HSV-компонентов через fromRGB
						ColorPickerSettings.Color = rgbColor
					end) 
				then 
					ColorPicker.HexInput.InputBox.Text = hex 
				end
				-- Вызываем callback с итоговым цветом
				pcall(function()ColorPickerSettings.Callback(Color3.fromHSV(h,s,v))end)
				-- ИСПРАВЛЕНИЕ: ранее здесь был баг — Color сохранялся как
				-- Color3.fromRGB(h*255, s*255, v*255) что неверно (h/s/v это [0..1] доли, не RGB).
				-- Color уже установлен корректно выше (rgbColor).
			...
			-- ИСПРАВЛЕНИЕ: .connect → .Connect (устаревший lowercase API, может не работать на некоторых executor'ах)
			...
			-- ИСПРАВЛЕНИЕ: ранее RunService.RenderStepped:connect (устаревший .connect!)
			-- никогда не отключался — постоянная утечка памяти и нагрузка на CPU.
			-- Теперь сохраняем соединение и отключаем при уничтожении элемента.
			...
			-- Отключаем RenderStepped-соединение при уничтожении ColorPicker-элемента
			-- чтобы не оставлять утечки памяти
		...
		function Tab:CreateLabel(LabelText : string, Icon: number, Color : Color3, IgnoreTheme : boolean)
			...
			-- УЛУЧШЕНИЕ: объединены два разрозненных блока иконки в один чистый блок
			if Icon and Label:FindFirstChild('Icon') then
				...
				-- Сдвигаем текст вправо чтобы освободить место для иконки
			...
			function LabelValue:Set(NewLabel, Icon, Color)
				...
				if Color then
					-- Обновляем цвет фона и обводки если передан новый цвет
					Label.BackgroundColor3 = Color or SelectedTheme.SecondaryElementBackground
					Label.UIStroke.Color = Color or SelectedTheme.SecondaryElementStroke
				end

				-- ИСПРАВЛЕНИЕ: ранее иконка устанавливалась дважды в дублирующихся блоках.
				-- Теперь один блок.
				if Icon and Label:FindFirstChild('Icon') then
					Label.Title.Position = UDim2.new(0, 45, 0.5, 0)
					Label.Title.Size = UDim2.new(1, -100, 0, 14)
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Unayfield | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check github for help with Unayfield specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			function InputSettings:Set(text)
				...
				-- ИСПРАВЛЕНИЕ: ранее pcall делался, но результат Success/Response игнорировался.
				-- Теперь показываем визуальную ошибку как во всех других элементах.
				local Success, Response = pcall(function()
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Unayfield | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check github for help with Unayfield specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			function DropdownSettings:Set(NewOption)
				...
				--SaveConfiguration() -- ИСПРАВЛЕНИЕ: раскомментировано — без этого Set() не сохранял конфиг
				if not DropdownSettings.Ext then
					SaveConfiguration()
				end
			end
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Unayfield | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check github for help with Unayfield specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Unayfield | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check github for help with Unayfield specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateSlider(SliderSettings)
			...
			-- ИСПРАВЛЕНИЕ: ранее формула была:
			-- (CurrentValue + Range[1]) / (Range[2] - Range[1])
			-- что неверно — Range[1] нужно вычитать (смещение от минимума), а не прибавлять.
			-- Правильная формула нормализации: (Value - Min) / (Max - Min)
			Slider.Main.Progress.Size = UDim2.new(0,
				(function()
					local fraction = (SliderSettings.CurrentValue - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])
					local px = Slider.Main.AbsoluteSize.X * fraction
					return math.max(px, 5) -- минимум 5px чтобы прогресс был виден
				end)(),
			1, 0)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Unayfield | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check github for help with Unayfield specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			function SliderSettings:Set(NewVal)
				...
				-- ИСПРАВЛЕНИЕ: та же формула что и при инициализации — (Value - Min) / (Max - Min)
				local fraction = (NewVal - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])
				local px = Slider.Main.AbsoluteSize.X * fraction
				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(0, math.max(px, 5), 1, 0)}):Play()
	...
	-- Алиас: Window:ChangeTheme(theme) → Window.ModifyTheme(theme)
	-- Некоторые скрипты вызывают ChangeTheme вместо ModifyTheme
	function Window:ChangeTheme(NewTheme)
		Window.ModifyTheme(NewTheme)
	end

	-- ToggleUI: переключает видимость интерфейса (показать/скрыть).
	-- Вызывается как Window:ToggleUI() из пользовательских скриптов.
	function Window:ToggleUI()
		setVisibility(not Hidden, not useMobileSizing)
	end
	...
	if not success then warn('Unayfield had an issue creating settings.') end
...
-- Алиас: RayfieldLibrary:ChangeTheme(theme)
-- Тестовые скрипты часто вызывают Rayfield:ChangeTheme() напрямую на объекте библиотеки,
-- а не на Window. Делегируем в локальную функцию ChangeTheme.
function RayfieldLibrary:ChangeTheme(NewTheme)
	local success = pcall(ChangeTheme, NewTheme)
	if not success then
		RayfieldLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
	end
...
-- Алиас: RayfieldLibrary:ToggleUI()
-- Переключает видимость интерфейса. Некоторые скрипты вызывают ToggleUI
-- на объекте библиотеки вместо Window.
...
function RayfieldLibrary:Destroy()
	...
	-- ИСПРАВЛЕНИЕ: nil-проверка на случай если Destroy() вызван до полной инициализации
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			-- Деселектируем все вкладки визуально
			for _, OtherTabButton in ipairs(TabList:GetChildren()) do
				if OtherTabButton.Name ~= "Template" and OtherTabButton.ClassName == "Frame" and OtherTabButton.Name ~= "Placeholder" then
					-- ИСПРАВЛЕНИЕ: ранее здесь была ссылка на переменную `TabButton` из
					-- замыкания последнего вызова CreateTab — это неверно, условие
					-- `OtherTabButton ~= TabButton` пропускало случайный таб.
					-- Теперь просто деселектируем всё кроме таба настроек.
					if OtherTabButton.Name ~= "Unayfield Settings" then
						TweenService:Create(OtherTabButton, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.TabBackground}):Play()
						TweenService:Create(OtherTabButton.Title, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {TextColor3 = SelectedTheme.TabTextColor}):Play()
						TweenService:Create(OtherTabButton.Image, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageColor3 = SelectedTheme.TabTextColor}):Play()
						TweenService:Create(OtherTabButton, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.7}):Play()
			...
			Elements.UIPageLayout:JumpTo(Elements['Unayfield Settings'])
		end)
	end)
...
Topbar.Hide.MouseButton1Click:Connect(function()
	-- ИСПРАВЛЕНИЕ: кнопка × (Hide) теперь полностью уничтожает UI через Destroy(),
	-- а не просто скрывает его в памяти. Это освобождает память и останавливает
	-- все RunService-соединения, созданные элементами (ColorPicker и т.д.).
	RayfieldLibrary:Destroy()
end)
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "Unayfield Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "Unayfield Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Unayfield Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "Unayfield Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
