...
local UnayfieldFolder = "Unayfield"
local ConfigurationFolder = UnayfieldFolder.."/Configurations"
...
local function loadSettings()
	...
	local success = pcall(function()
		task.spawn(function()
			if callSafely(isfolder, UnayfieldFolder) then
				if callSafely(isfile, UnayfieldFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, UnayfieldFolder..'/settings'..ConfigurationExtension)
				end
			end

			if useStudio then
				file = [[{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"Unayfield Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Unayfield Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}}}]]
			end

			if file then
				local ok, decoded = pcall(function() return HttpService:JSONDecode(file) end)
				file = ok and decoded or {}
			else
				file = {}
			end
	...
	if not success then
		if writefile then
			warn('Unayfield | Issue accessing configuration saving capability.')
		end
	end
end
...
local SelectedTheme = UnayfieldLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = UnayfieldLibrary.Theme[Theme]
...
local function getIcon(name)
	...
	if not r then error("Unayfield Icons: Failed to find icon by the name of \"" .. name .. "\"", 2) end
	local rirs = r[2]
	local riro = r[3]
	if type(r[1]) ~= "number" or type(rirs) ~= "table" or type(riro) ~= "table" then
		error("Unayfield Icons: Internal error: Invalid auto-generated asset entry")
	end
	return {id = r[1], imageRectSize = Vector2.new(rirs[1], rirs[2]), imageRectOffset = Vector2.new(riro[1], riro[2])}
end
...
local function LoadConfiguration(Configuration)
	...
	local changed
	if not success then warn('Unayfield | Issue decoding configuration file.') return end
	for FlagName, Flag in pairs(UnayfieldLibrary.Flags) do
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(UnayfieldLibrary.Flags) do
		if v.Type == "ColorPicker" then
			Data[i] = PackColor(v.Color)
		else
			if typeof(v.CurrentValue) == 'boolean' then
				Data[i] = v.CurrentValue == false and false or (v.CurrentValue or v.CurrentKeybind or v.CurrentOption or v.Color)
			else
...
function UnayfieldLibrary:Notify(data)
	task.spawn(function()
...
local function Hide(notify)
	...
	if notify then
		if useMobilePrompt then
			UnayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, tap 'Show' to unhide.", Duration = 7, Image = 4400697855})
		else
			UnayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, press " .. tostring(getSetting("General", "rayfieldOpen")) .. " to unhide.", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings()
	...
	if success then
		...
		callSafely(writefile, UnayfieldFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
function UnayfieldLibrary:CreateWindow(Settings)
	if Rayfield:FindFirstChild('Loading') then
		if getgenv and not getgenv().unayfieldCached then
	...
	if getgenv then getgenv().unayfieldCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, function()
			UnayfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'Unayfield may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})
		end)
	end
	...
	if Settings.LoadingTitle ~= "Unayfield Interface Suite" then
		LoadingFrame.Version.Text = "Unayfield UI"
	end
	...
	if Settings.Theme then
		...
		if not success then
			local ok2, r2 = pcall(ChangeTheme, 'Default')
			if not ok2 then warn('Unayfield | CRITICAL ERROR - NO DEFAULT THEME') end
		end
	end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(UnayfieldFolder.."/Discord Invites")
		local requestFunc = (syn and syn.request) or (fluxus and fluxus.request) or (http and http.request) or http_request or request
		if not callSafely(isfile, UnayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then
				callSafely(writefile, UnayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension, "RememberJoins")
			end
		end
	end
	...
	if Settings.KeySystem then
		if not Settings.KeySettings then
			...
			ensureFolder(UnayfieldFolder.."/Key System")
			...
			if Settings.KeySettings.GrabKeyFromSite then
				for i, Key in ipairs(Settings.KeySettings.Key) do
					...
					if not Success then
						warn('Unayfield | Key fetch error: '..tostring(Response))
					end
				end
			end
			...
			if callSafely(isfile, UnayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
				for _, MKey in ipairs(Settings.KeySettings.Key) do
					local savedKeys = callSafely(readfile, UnayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
					if savedKeys and string.find(savedKeys, MKey, 1, true) then
						Passthrough = true
					end
				end
			end
				...
				KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
					...
					if KeyFound then
						...
						if Settings.KeySettings.SaveKey then
							callSafely(writefile, UnayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
							UnayfieldLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved.", Image = 3605522284})
						end
					else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					warn('Unayfield | '..ButtonSettings.Name..' Callback Error: '..tostring(Response))
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings)
			...
			CPMain.Image = "http://www.roblox.com/asset/?id=11415645739"
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(CPMain, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= UnayfieldLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving and Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
				UnayfieldLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
			end
		...
		function Tab:CreateLabel(LabelText, Icon, Color, IgnoreTheme)
			...
			local function applyIcon(iconVal)
				if iconVal then
					if typeof(iconVal) == 'string' and Icons then
						local asset = getIcon(iconVal)
						if asset then
						...
						Label.Icon.Image = getAssetUri(iconVal)
					end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					warn('Unayfield | '..InputSettings.Name..' Callback Error: '..tostring(Response))
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving and Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
				UnayfieldLibrary.Flags[InputSettings.Flag] = InputSettings
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			Dropdown.Name = string.find(DropdownSettings.Name, "closed") and "Dropdown" or DropdownSettings.Name
			...
			for _, unusedoption in ipairs(Dropdown.List:GetChildren()) do
				if unusedoption.ClassName == "Frame" and unusedoption.Name ~= "Placeholder" then
					unusedoption:Destroy()
				end
			end
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							warn('Unayfield | '..DropdownSettings.Name..' Callback Error: '..tostring(Response))
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			function DropdownSettings:Set(NewOption)
				DropdownSettings.CurrentOption = typeof(NewOption) == "string" and {NewOption} or NewOption
				if not DropdownSettings.MultipleOptions then
					DropdownSettings.CurrentOption = {DropdownSettings.CurrentOption[1]}
				end
				if DropdownSettings.MultipleOptions then
			...
			if Settings.ConfigurationSaving and Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
				UnayfieldLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							warn('Unayfield | '..KeybindSettings.Name..' Callback Error: '..tostring(Response))
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving and Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
				UnayfieldLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= UnayfieldLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					warn('Unayfield | '..ToggleSettings.Name..' Callback Error: '..tostring(Response))
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving and Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					UnayfieldLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Main.Information.Text = SliderSettings.Suffix and tostring(SliderSettings.CurrentValue).." "..SliderSettings.Suffix or tostring(SliderSettings.CurrentValue)
			...
			Slider.Main.Interact.MouseButton1Down:Connect(function(X)
				...
				local Loop; Loop = RunService.Stepped:Connect(function()
					if SLDragging then
						...
						Slider.Main.Information.Text = SliderSettings.Suffix and tostring(NewValue).." "..SliderSettings.Suffix or tostring(NewValue)
						if SliderSettings.CurrentValue ~= NewValue then
							local Success, Response = pcall(function() SliderSettings.Callback(NewValue) end)
							if not Success then
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
								Slider.Title.Text = "Callback Error"
								warn('Unayfield | '..SliderSettings.Name..' Callback Error: '..tostring(Response))
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
							end
			...
			function SliderSettings:Set(NewVal)
				local clamped = math.clamp(NewVal, SliderSettings.Range[1], SliderSettings.Range[2])
				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(0, Slider.Main.AbsoluteSize.X * ((clamped + SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])) > 5 and Slider.Main.AbsoluteSize.X * (clamped / (SliderSettings.Range[2] - SliderSettings.Range[1])) or 5, 1, 0)}):Play()
				Slider.Main.Information.Text = tostring(clamped) .. " " .. (SliderSettings.Suffix or "")
				local Success, Response = pcall(function() SliderSettings.Callback(clamped) end)
				if not Success then
					TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = Color3.fromRGB(85, 0, 0)}):Play()
					Slider.Title.Text = "Callback Error"
					warn('Unayfield | '..SliderSettings.Name..' Callback Error: '..tostring(Response))
				...
				SliderSettings.CurrentValue = clamped
				if not SliderSettings.Ext then SaveConfiguration() end
			end

			if Settings.ConfigurationSaving and Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
				UnayfieldLibrary.Flags[SliderSettings.Flag] = SliderSettings
			end

			Rayfield.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				if SelectedTheme ~= UnayfieldLibrary.Theme.Default then Slider.Main.Shadow.Visible = false end
				Slider.Main.BackgroundColor3 = SelectedTheme.SliderBackground
				Slider.Main.UIStroke.Color = SelectedTheme.SliderStroke
				Slider.Main.Progress.UIStroke.Color = SelectedTheme.SliderStroke
				Slider.Main.Progress.BackgroundColor3 = SelectedTheme.SliderProgress
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			UnayfieldLibrary:Notify({Title = 'Unable to Change Theme', Content = 'Theme not found.', Image = 4400704299})
		else
			UnayfieldLibrary:Notify({Title = 'Theme Changed', Content = 'Theme changed to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end

	local ok = pcall(createSettings, Window)
	if not ok then warn('Unayfield | Issue creating settings panel.') end
...
function UnayfieldLibrary:SetVisibility(visibility)
	setVisibility(visibility, false)
end

function UnayfieldLibrary:IsVisible()
	return not Hidden
end

local hideHotkeyConnection
function UnayfieldLibrary:Destroy()
...
function UnayfieldLibrary:LoadConfiguration()
	if useStudio then
		local config = [[{"Toggle1adwawd":true,"ColorPicker1awd":{"B":255,"G":255,"R":255},"Slider1dawd":100}]]
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			if isfile then
				...
				notified = true
				UnayfieldLibrary:Notify({Title = "Unayfield Configurations", Content = "Configuration saving requires filesystem support.", Image = 4384402990})
			end
		end)
		if success and loaded and not notified then
			UnayfieldLibrary:Notify({Title = "Unayfield Configurations", Content = "Configuration loaded from previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Unayfield Configurations Error | '..tostring(result))
			UnayfieldLibrary:Notify({Title = "Unayfield Configurations", Content = "Issue loading configuration. Check Developer Console.", Image = 4384402990})
		end
	end
	globalLoaded = true
end
...
task.delay(4, function()
	UnayfieldLibrary.LoadConfiguration()
